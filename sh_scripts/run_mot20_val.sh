...
# default: run pkf
if [ $# -eq 0 ]; then
    python tools/run_mot.py -f exps/example/mot/yolox_x_mot20_half.py \
        -c pretrained/ocsort_x_mot20.pth.tar -b 1 -d 1 --fp16 --fuse --track_thresh 0.4 \
        --mot20 --expn pkf --ambig_thresh 0.95 --update_weight_thresh 0.25 --use_saved_dets
    exit 1
fi
...
if [ $# -ne 0 ]; then
    ...
    python tools/run_mot.py --alg_name $1 -f exps/example/mot/yolox_x_mot20_half.py \
        -c pretrained/ocsort_x_mot20.pth.tar -b 1 -d 1 --fp16 --fuse --track_thresh 0.4 \
        --mot20 --expn $1 --use_saved_dets
    exit 1
fi

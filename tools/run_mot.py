...
from utils.data import get_yolox_datadir
...
def main(exp, args, num_gpu):
    ...
    elif exp.val_ann == "train.json":
        gt_type = ''
        seqs = "MOT20-train" if args.mot20 else "MOT17-train"
    ...
    # start evaluate
    if args.alg_name == "pkf":
        *_, summary = evaluator.evaluate_pkf(
            model, is_distributed, args.fp16, trt_file, decoder, exp.test_size, results_folder
        )
    elif args.alg_name == "ocsort":
        *_, summary = evaluator.evaluate_ocsort(
            model, is_distributed, args.fp16, trt_file, decoder, exp.test_size, results_folder
        )
    elif args.alg_name == "bytetrack":
    ...
    if args.mot20:
        gtfiles = glob.glob(
            os.path.join(get_yolox_datadir(), 'MOT20/train', '*/gt/gt{}.txt'.format(gt_type)))
    else:
        gtfiles = glob.glob(
            os.path.join(get_yolox_datadir(), 'mot/train', '*/gt/gt{}.txt'.format(gt_type)))
    ...
    gt = OrderedDict([(Path(f).parts[-3], mmp.io.loadtxt(f, fmt='mot15-2D', min_confidence=1, sep=',')) for f in gtfiles])
    ts = OrderedDict([(os.path.splitext(Path(f).parts[-1])[0], mmp.io.loadtxt(f, fmt='mot15-2D', min_confidence=-1, sep=',')) for f in tsfiles if "detections" not in f])    

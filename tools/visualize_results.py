...
DANCETRACK_VAL_SPLIT = dict()
for video_name in DANCETRACK_VAL_VIDEO_LEN:
    num_images = DANCETRACK_VAL_VIDEO_LEN[video_name]
    DANCETRACK_VAL_SPLIT[video_name] = dict()
    DANCETRACK_VAL_SPLIT[video_name]["full"] = [1, num_images]
...
def visualize_detections(img_dir, out_dir, detections_dir, mode="val_half", path="{}/{}_detections.txt", dataset="mot17", test=False):
    ...
    if "mot17" in dataset:
        ...
        VIDEO_LEN = DANCETRACK_VAL_VIDEO_LEN
        VIDEO_SPLIT = DANCETRACK_VAL_SPLIT
    else:
        assert 0
...
def visualize_tracks(img_dir, out_dir, tracks_dir, mode, dataset="mot17"):
    ...
    for video_name in VIDEO_LEN:
        ...
        cmd = "ffmpeg -framerate 5 -pattern_type glob -i '{}/*.jpg' {}/{}.mp4".format(video_out_dir, out_dir, video_name)
        os.popen(cmd)


def visualize_gt(dataset, img_dir, out_dir):
    if "mot17" in dataset:
        VIDEO_LEN = MOT17_VIDEO_LEN
    elif dataset == "mot20":
        VIDEO_LEN = MOT20_VIDEO_LEN
    ...
    for video_name in VIDEO_LEN:
        ...
        for frame_ind in range(fake_frame_min, fake_frame_max+1):
            ...
            cv2.imwrite(os.path.join(video_out_dir, "%06d.jpg" % real_frame_ind), img)
        # cmd = "ffmpeg -framerate 5 -pattern_type glob -i '{}/*.jpg' -c:v libx264 -pix_fmt yuv420p {}/{}.mp4".format(video_out_dir, out_dir, video_name)
        cmd = "ffmpeg -framerate 5 -pattern_type glob -i '{}/*.jpg' -pix_fmt yuv420p {}/{}.mp4".format(video_out_dir, out_dir, video_name)
        os.popen(cmd)
...
def merge_visualization(det_dir, track_dir, gt_dir, out_dir):
    ...
    for seq in seqs:
        ...
        cmd = "ffmpeg -framerate 5 -pattern_type glob -i '{}/*.jpg' -pix_fmt yuv420p {}/merged_{}.mp4".format(seq_out_dir, out_dir, seq)
        os.popen(cmd)
...
if __name__ == "__main__":
    args = make_parser()
    data_root = '/home/hanwen/shared/data/tracking_datasets'
    if "mot17" in args.dataset:
        img_dir = os.path.join(data_root, "mot/train")
    elif args.dataset == "mot20":
        img_dir = os.path.join(data_root, "mot20/test")
    elif args.dataset == "dancetrack_val":
        img_dir = os.path.join(data_root, "dancetrack/val")
    elif args.dataset == "dancetrack_test":
        img_dir = os.path.join(data_root, "dancetrack/test")
    ...
    if args.vis == "det":
        visualize_detections(img_dir, out_dir, res_dir, mode=args.mode, dataset=args.dataset)
    elif args.vis == "track":
        ...
        det_dir = os.path.join(out_src_dir, args.dataset, exp_name, 'det')
        # track_dir = "visualizations/yolox_x_ablation/{}/track".format(args.exp_name)
        track_dir = os.path.join(out_src_dir, args.dataset, exp_name, 'track')
        # gt_dir = "visualizations/GTs"
        gt_dir = os.path.join(out_src_dir, args.dataset, exp_name, 'gt')
        # out_dir = "visualizations/yolox_x_ablation/{}/merged".format(args.exp_name)
        out_dir = os.path.join(out_src_dir, args.dataset, exp_name, 'merged')
        ...
        out_dir = os.path.join(out_src_dir, args.dataset, exp_name, 'gt')
        os.makedirs(out_dir, exist_ok=True)
        visualize_gt(args.dataset, img_dir, out_dir)

...
from trackers.pkf_tracker.pkf import PKFTracker
    ...
    def evaluate_pkf(
        ...
        for cur_iter, (imgs, _, info_imgs, ids) in enumerate(
            ...
            with torch.no_grad():
                ...
                if self.args.test:
                    split = 'MOT20-test' if self.args.mot20 else 'MOT17-test'
                elif self.args.train:
                    split = 'MOT20-train' if self.args.mot20 else 'MOT17-train'
                else:
                    split = 'MOT20-val' if self.args.mot20 else 'MOT17-val'

                det_file_dir = os.path.join(self.args.detection_dir, split, video_name)
                def_fname = os.path.join(det_file_dir, '%08d.pkl' % frame_id)
                # skip the the last iters since batchsize might be not enough for batch inference
                is_time_record = cur_iter < len(self.dataloader) - 1
                if is_time_record:
                    start = time.time()
                    
                if os.path.exists(def_fname) and self.args.use_saved_dets:
                    outputs = torch.load(def_fname)
                    ...
                    if self.args.save_detections:
                        os.makedirs(det_file_dir, exist_ok=True)
                        torch.save(outputs, def_fname)
        ...
        for cur_iter, (imgs, _, info_imgs, ids) in enumerate(
            ...
            with torch.no_grad():
                ...
                if self.args.test:
                    ...
                    split = 'MOT17-val'
                det_file_dir = os.path.join(self.args.detection_dir, split, video_name)
                det_fname = os.path.join(det_file_dir, '%08d.pkl' % frame_id)
                # skip the the last iters since batchsize might be not enough for batch inference
                is_time_record = cur_iter < len(self.dataloader) - 1
                if is_time_record:
                    start = time.time()
                
                if os.path.exists(det_fname) and self.args.use_saved_dets:
                    outputs = torch.load(det_fname)

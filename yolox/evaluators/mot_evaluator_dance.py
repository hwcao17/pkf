    ...
    def evaluate_pkf(
        ...
        for cur_iter, (imgs, _, info_imgs, ids) in enumerate(progress_bar(self.dataloader)):
            ...
            with torch.no_grad():
                ...
                split = 'test' if self.args.test else 'val'
                det_file =  os.path.join(self.args.detection_dir, split, video_name, '%08d.pkl' % frame_id)
                
                if os.path.exists(det_file) and self.args.use_saved_dets:
                    outputs = torch.load(det_file)
                    ...
                    if self.args.save_detections:
                        save_dir = os.path.join(self.args.detection_dir, split, video_name)
                        os.makedirs(save_dir, exist_ok=True)
                        save_fname = os.path.join(save_dir, "%08d.pkl" % frame_id)
                        torch.save(outputs, save_fname)
